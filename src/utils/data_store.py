...
class OrderDataStore:
    ...
    def _suggest_product_ids(self, invalid_id: str) -> list[str]:
        normalized_invalid = _normalize(invalid_id)
        suggestions = []
        for product_id, product in self.product_index.items():
            if normalized_invalid in _normalize(product_id) or _normalize(product_id) in normalized_invalid:
    ...
    def get_product_details(self, product_ids: list[str]) -> dict:
        ...
        for product_id in product_ids:
            ...
            if not product:
                suggestions = self._suggest_product_ids(product_id)
                suggestion_text = f" Did you mean: {', '.join(suggestions)}?" if suggestions else ""
                details.append({
                    "product_id": product_id, 
                    "status": "not_found", 
                    "message": f"Product ID {product_id} not found.{suggestion_text}"
                })
                continue
            details.append(
                {
    ...
    def calculate_order_totals(self, *, items: list[OrderLineInput], detail_token: str, discount_rate: float) -> dict:
        ...
        for item in sorted(items, key=lambda current: current.product_id):
            ...
            if not product:
                suggestions = self._suggest_product_ids(item.product_id)
                suggestion_text = f" Did you mean: {', '.join(suggestions)}?" if suggestions else ""
                errors.append(f"Unknown product_id: {item.product_id}.{suggestion_text}")
                continue
